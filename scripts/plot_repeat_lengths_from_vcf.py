...
try:
    from privacy import hexamer_carriers, weird_outlier_carrier
except ImportError:
    hexamer_carriers = []
    weird_outlier_carrier = "unknown"
...
def main():
            ...
            for variant in VCF(vcf):
                ...
                if variant.INFO.get("OUTLIERS") is not None:
                    for outlier in variant.INFO.get("OUTLIERS").split(","):
                        if (
                            not name == weird_outlier_carrier
    ...
    if args.mark_hexamers:
        ...
        # add a star symbol as annotation to specific line
        for name in hexamer_carriers:
